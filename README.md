...
- **Alert engine** (`internal/alert`) — per-target windows of recent loss/latency samples, with hysteresis matchers (`CheckLoss`, `CheckLatency`: raise after X bad rounds, clear after X good) and a pattern DSL (`>50%,>50%` / `>200,>200` shape matches). Firing/resolved state with edge-triggering; notifiers = log + webhook (JSON POST). Verified firing live on real loss and latency. Alerts are defined in config and attached to targets by name.
